...
void convolve2D(std::vector<RGBA> &data, int width, int height, const std::vector<float> &kernel) {
    // Task 2: initialize a vector, called `result`, to temporarily store your output image data

    // Task 3: obtain the kernel's dimensions
    ...
    for (int r = 0; r < height; r++) {
        for (int c = 0; c < width; c++) {
            ...
            // 2. Iterate over the kernel using its dimensions from task 3.
            ...
            // Task 6: Update buffer with the new RGBA pixel value created from
            //          redAcc, greenAcc, and blueAcc
        }
    }

    // Task 7: Copy the RGBA data from `result` (task 2) to `data`
}
